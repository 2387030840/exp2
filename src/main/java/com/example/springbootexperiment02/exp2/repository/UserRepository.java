package com.example.springbootexperiment02.exp2.repository;

import com.example.springbootexperiment02.exp2.entity.Address;
import com.example.springbootexperiment02.exp2.entity.User;
import org.springframework.stereotype.Repository;
import org.springframework.transaction.annotation.Transactional;

import javax.persistence.EntityManager;
import javax.persistence.PersistenceContext;
import java.util.ArrayList;
import java.util.List;

@Repository
@Transactional
public class UserRepository {
    @PersistenceContext
    private EntityManager em;

    /**
     * 添加用户，并返回包括数据库时间戳的用户对象
     * @param user
     * @return
     */
    public User addUser(User user) {

        em.persist(user);
        em.refresh(user);
        return user;
    }

    /**
     * 添加地址，并指定地址对应的用户
     * @param address
     * @param uid
     * @return
     */
    public Address addAddress(Address address, int uid) {
        address.setUser(em.find(User.class, uid));
        em.persist(address);
        em.find(User.class, uid).getAddresses().add(address);
        em.flush();
        em.refresh(address);
        //System.out.println(em.find(User.class, uid).getAddresses());
        return address;
    }

    /**
     * 更新指定ID用户的姓名
     * @param uid
     * @param newName
     * @return
     */
    public User updateUser(int uid, String newName) {
        em.find(User.class, uid).setName(newName);
        em.flush();
        return em.find(User.class , uid);
    }

    /**
     * 尝试使用merge()，以及find()2种方法分别实现
     * 更新指定地址为指定用户
     * @param aid
     * @param uid
     * @return
     */
    public Address updateAddress(int aid, int uid) {
        Address address = new Address();
        address.setId(aid);
        Address afterMergeAdd = em.merge(address);
        em.refresh(afterMergeAdd);
        afterMergeAdd.setUser(em.find(User.class, uid));
        return em.find(Address.class, aid);
    }

    /**
     * 返回指定用户的全部地址，没有返回空集合，而非null
     * @param uid
     * @return
     */
    public List<Address> listAddresses(int uid) {
        User user = em.find(User.class, uid);
        List<Address> addressList = new ArrayList<>();
        List<Address> getList = user.getAddresses();
        for(Address address:getList) {
            addressList.add(address);
        }
        return addressList;
    }

    public void removeAddress(int aid) {
        em.remove(em.find(Address.class, aid));
    }

    /**
     * 删除用户，设置级联操作或手动删除相关地址
     * @param uid
     */
    public void removeUser(int uid) {
        em.remove(em.find(User.class, uid));
    }

}
